      ...
      <table width="100%" border="0" align="center" cellspacing="0" cellpadding="0" bgcolor="#FFFFFF">
        ...
        <tr align="right"> 
		  ...
		  <table align="center" width="100%" border="0" cellpadding="0" cellspacing="3" height="20">
			  ...
			  <td height="22"><font face="cooper black, Helvetica, sans-serif" size="3"><a href="editmed.php"><b>BACK</b></a></font></td> 
...
<?php /*         VIEW.PHP        Displays all data from 'players' table*/        // connect to the database     
 include"bdd.php";       
  // get results from database   
       $result = mysql_query("select * from clinic.medication where  medication.patientid = '".$_POST["medicalnumber"]."' and medication.orderdate= '".$_POST["orderdate"]."'")   or die(mysql_error());       
	                      // display data in table       
						        
						             echo "<table border='1' bgcolor='white' cellpadding='10'>";       
									  echo "<tr> <th>Id</th> <th>MR</th> <th>DOCTOR NAME</th><th>ORDER DATE </th><th>MEDICATION NAME </th><th>PRESCRIPTION</th><th>EDIT </th><th>DELETE</th> </tr>";     
						      // loop through results of database query, displaying them in the table    
							      while($row = mysql_fetch_array( $result )) {                         
								         // echo out the contents of each row into a table       
										          echo "<tr>";      
												            echo '<td>' . $row['medicationid'] . '</td>';       
															         echo '<td>' . $row['patientid'] . '</td>';       
																	          echo '<td>' . $row['doctorname'] . '</td>';       
																			  echo '<td>' . $row['orderdate'] . '</td>';   
																			  echo '<td>' . $row['medicationname'] . '</td>';   
																			  echo '<td>' . $row['prescription'] . '</td>';   
																			           echo '<td><a href="edit1.php?medicationid=' . $row['medicationid'] . '"><u>Edit</u></a></td>';          
																					         echo '<td><a href="delete1.php?medicationid=' . $row['medicationid'] . '"><u>Delete</u></a></td>';                                                                                      echo "</tr>";         }       
												    // close table>       
													 echo "</table>";?><p><a href="medication.php">Add a new record</a></p></body></html> 