        ...
        <tr align="right"> 
          <td colspan="2" height="9"> 
            ...
            <table align="center" width="100%" border="0" cellpadding="0" cellspacing="3" height="40">
			  ...
			  <td height="22"><font face="cooper black, Helvetica, sans-serif" size="3"><a href="editvisit.php"><b>BACK</b></a></font></td> 
			   <td height="22"><font face="cooper black, Helvetica, sans-serif" size="3"><a href="datagrid.php"><b>HOME</b></a></font></td> 
			   
				   <td height="22"><font face="cooper black, Helvetica, sans-serif" size="3"><b>CHECK VISIT TO EDIT</b>
                </font></td>
...
<?php /*         VIEW.PHP        Displays all data from 'players' table*/        // connect to the database     
 include"bdd.php";       
  // get results from database   
       $result = mysql_query("select * from clinic.investigation where  investigation.patientid = '".$_POST["medicalnumber"]."' and investigation.date= '".$_POST["orderdate"]."'")   or die(mysql_error());       
	                      // display data in table       
						        
						             echo "<table  bgcolor='white' border='1' cellpadding='10'>";       
									  echo "<tr> <th>TRANS ID</th> <th>MR</th> <th>DATE</th><th>HISTORY</th><th>CHIEF COMPLAINT</th><th>PHYSICAL EXAM</th><th>DIAGNOSIS</th><th>EDIT </th><th>DELETE</th> </tr>";     
						      // loop through results of database query, displaying them in the table    
							      while($row = mysql_fetch_array( $result )) {                         
								         // echo out the contents of each row into a table       
										          echo "<tr>";      
												            echo '<td>' . $row['investigationid'] . '</td>';       
															         echo '<td>' . $row['patientid'] . '</td>';       
																	          echo '<td><center>' . $row['date'] . '</td>';       
																			  echo '<td><center>' . $row['history'] . '</td>';   
																			  echo '<td><center>' . $row['chiefcomplaint'] . '</td>';   
																			  echo '<td><center>' . $row['physicalexam'] . '</td>';  
																			  echo '<td>' . $row['diagnosis'] . '</td>';  
																			           echo '<td><a href="edit2.php?investigationid=' . $row['investigationid'] . '"><u>Edit</u></a></td>';          
																					         echo '<td><a href="delete2.php?investigationid=' . $row['investigationid'] . '"><u>Delete</u></a></td>';                                                                                      echo "</tr>";         }       
												    // close table>       
													 echo "</table>";?><p><a href="medical1.php">Add a new record</a></p></body></html> 