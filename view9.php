        ...
        <tr align="right"> 
          <td colspan="2" height="9"> 
            ...
            <table align="center" width="100%" border="0" cellpadding="0" cellspacing="3" height="40">
			  ...
			  <td height="22"><font face="cooper black, Helvetica, sans-serif" size="3"><a href="editsick.php"><b>BACK</b></a></font></td> 
			   <td height="22"><font face="cooper black, Helvetica, sans-serif" size="3"><a href="datagrid2.php"><b>HOME</b></a></font></td> 
			   
				   <td height="22"><font face="cooper black, Helvetica, sans-serif" size="3"><b>UPDATE SICK LEAVE INFORMATIONS</b>
                  </font></td>
...
<?php /*         VIEW.PHP        Displays all data from 'players' table*/        // connect to the database     
 include"bdd.php";       
  // get results from database   
       $result = mysql_query("select * from clinic.sick where  sick.patientid = '".$_POST["medicalnumber"]."' and sick.date= '".$_POST["orderdate"]."'")   or die(mysql_error());       
	                      // display data in table       
						        
						             echo "<table  bgcolor='white' border='1' cellpadding='10'>";       
									  echo "<tr><th>ID</th><th>MR</th><th>DIAGNOSIS</th><th>DAY'S NUMBER</th><th>FROM DATE</th><th>TO DATE</th><th>SCIK LEAVE DATE</th><th>EDIT </th><th>DELETE</th> </tr>";     
						      // loop through results of database query, displaying them in the table    
							      while($row = mysql_fetch_array( $result )) {                         
								         // echo out the contents of each row into a table       
										          echo "<tr>";      
												            echo '<td>' . $row['sickid'] . '</td>';       
															         echo '<td>' . $row['patientid'] . '</td>';       
																	             
																			  echo '<td><center>' . $row['diagnosis'] . '</td>';   
																			  echo '<td><center>' . $row['day'] . '</td>';   
																			  echo '<td><center>' . $row['fromdate'] . '</td>';  
																			  echo '<td>' . $row['todate'] . '</td>';  
																			    echo '<td>' . $row['date'] . '</td>';
																			           echo '<td><a href="edit9.php?sickid=' . $row['sickid'] . '"><u>Edit</u></a></td>';          
																					         echo '<td><a href="delete9.php?sickid=' . $row['sickid'] . '"><u>Delete</u></a></td>';                                                                                      echo "</tr>";         }       
												    // close table>       
													 echo "</table>";?><p><a href="sick.php">Add a new record</a></p></body></html> 